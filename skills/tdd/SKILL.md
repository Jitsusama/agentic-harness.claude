...
description: >
  ...
  commit cadence. Use when implementing with TDD or writing
  tests first, and drive the loop through the
  agentic-harness-core CLI.
---
...
## The agentic-harness-core CLI

`agentic-harness-core tdd` tracks one red-green-refactor loop at
a time. It is a tracker and a reminder, not a gate. It never
blocks your file writes and there is no prompt to wait on: every
call returns immediately with a JSON result you read and act on
directly. There is no separate status line or widget here — the
JSON response is the whole surface, so read `outcome`,
`discipline`/`guidance` and `loop` on every call rather than
assuming the loop advanced.

Run it from the project root so its state file
(`.agentic-harness/tdd-loop.json`, created on first use) stays
with the project. Add `.agentic-harness/` to the project's
`.gitignore` if it isn't already ignored.

Attest a transition by piping the attestation as JSON to `tdd
attest`:

```sh
echo '{"action":"plan","behaviour":"rejects an empty cart"}' \
  | agentic-harness-core tdd attest
```

A landed transition looks like:

```json
{"outcome":"advanced","loop":{"phase":"plan","assertionFailure":false,"behaviour":"rejects an empty cart","iteration":1},"discipline":"One increment. Describe the intent: the behaviour you want, not the code that already exists."}
```

A refused one changes nothing and hands back guidance instead:

```json
{"outcome":"refused","loop":{"phase":"idle","assertionFailure":false,"behaviour":null,"iteration":0},"guidance":"Name the single behaviour under test: the exported thing you want to exist. One increment per loop."}
```

Read `outcome` first, then act on `discipline` (advanced) or
`guidance` (refused) before doing anything else. Check where the
loop currently stands, e.g. after resuming a session, with:

```sh
agentic-harness-core tdd status
```

### The Transitions

The classic three steps expand into the CLI's transitions. Red
...
"assertion"`. The CLI records the kind you report; it never
